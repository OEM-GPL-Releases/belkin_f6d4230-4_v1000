...
static int nf_conntrack_reserved  __read_mostly = 512;
static atomic_t nf_conntrack_count_unreserved = ATOMIC_INIT(0);
#define RESERVED_CT(ct) (IPS_RESERVED == (ct)->reserve.mask)
static int is_reserved( int portnum)
{	
...
static int regardless_drop_unreserved(struct list_head *chain)
{
		     ...
		     u_int32_t features)
{
	...
	if (nf_conntrack_max
		...
		unsigned int hash = hash_conntrack(orig);
    		...
    		if (!early_drop(&nf_conntrack_hash[hash])) {
                ...
                if (!early_drop(&nf_conntrack_hash[hash])
                    && !early_drop(&nf_conntrack_hash[next])) 
                {
                        if (nf_conntrack_reserved 
                            && is_reserved( port_num)
	                    && atomic_read(&nf_conntrack_count_unreserved) > (nf_conntrack_max - nf_conntrack_reserved))
                        {
                                unsigned int search_begin = next;

                                while (!regardless_drop_unreserved(&nf_conntrack_hash[next]))
                                {
                                        next = (drop_next++)%nf_conntrack_htable_size;

                                        if(next == search_begin)
                                        {
                                                atomic_dec(&nf_conntrack_count);
                                                if (net_ratelimit())
                                                	printk(KERN_WARNING
                                                	       "nf_conntrack: table full, regardless drop fail, dropping"
                                                	       " packet.\n");
                                                return ERR_PTR(-ENOMEM);
                                        }
                                }
	...
	read_unlock_bh(&nf_ct_cache_lock);
        ...
        if(nf_conntrack_reserved)    
        {        
            if (!is_reserved( port_num))	// not a reserve port    	
            {    		
                atomic_inc(&nf_conntrack_count_unreserved);    		
                conntrack->reserve.mask = 0;    	
            }    	
            else    	
            {    		
                conntrack->reserve.mask = IPS_RESERVED;
            }    
        }
#endif // _SC_BUILD_
...
void nf_conntrack_free(struct nf_conn *conntrack)
{
	...
	DEBUGP("nf_conntrack_free: features = 0x%x, conntrack=%p\n", features,
        ...
        if(nf_conntrack_reserved)    
        {        
            if( !RESERVED_CT(conntrack) )
                atomic_dec(&nf_conntrack_count_unreserved);    
        }
#endif
...
static struct proc_dir_entry *nf_conntrack_reserve_proc;
static ssize_t proc_read_nf_conntrack_reserve_fops(struct file *filp, char *buf, size_t count , loff_t *offp)
{    
    char buff[30];
    ...
    sprintf(buff, "%d %d %d\n", nf_conntrack_reserved, atomic_read(&nf_conntrack_count_unreserved), atomic_read(&nf_conntrack_count));
    copy_to_user(buf, buff, strlen(buff));
    *offp = strlen(buff);
    
    return *offp;
}

static ssize_t proc_write_nf_conntrack_reserve_fops(struct file *filp,const char *buffer, size_t count , loff_t *offp)
{
    int len;
    char value_s[10];
    ...
    if (copy_from_user(&value_s, buffer, len))
			return -EFAULT;

    value_s[sizeof(value_s) - 1] = '\0';
    nf_conntrack_reserved = simple_strtoul(value_s, NULL, 0);
    
    printk("reserved conntrack set to  %d\n", nf_conntrack_reserved);
    
    return count;
}
struct file_operations nf_conntrack_reserve_fops = 
{        
    read: proc_read_nf_conntrack_reserve_fops,
    write: proc_write_nf_conntrack_reserve_fops,
};
#endif
...
int __init nf_conntrack_init(void)
{
	...
	int ret;
       ...
       nf_conntrack_reserve_proc = create_proc_entry("nf_conntrack_reserved",0666,&proc_root);
       nf_conntrack_reserve_proc->owner = THIS_MODULE;	
       nf_conntrack_reserve_proc->proc_fops = &nf_conntrack_reserve_fops;
#endif
