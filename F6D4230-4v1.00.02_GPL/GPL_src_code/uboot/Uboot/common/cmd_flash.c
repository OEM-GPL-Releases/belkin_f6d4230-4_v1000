...
int do_flerase (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (strcmp(argv[1], "linux") == 0) 
	{
		...
		rcode = flash_sect_erase(CFG_FLASH_BASE+0x50000, CFG_FLASH_BASE+0x3FFFFF);
	
		return rcode;
	}
	else if (strcmp(argv[1], "cramfs") == 0) 
	{
		printf("\n Erase File System block !!\n");
		printf("From 0x%X To 0x%X\n", CFG_FLASH_BASE+0x530000, CFG_FLASH_BASE+0x7FFFFF);
		rcode = flash_sect_erase(CFG_FLASH_BASE+0x530000, CFG_FLASH_BASE+0x7FFFFF);
	...
	else if (strcmp(argv[1], "uboot") == 0) 
	{
		...
		rcode = flash_sect_erase(CFG_FLASH_BASE, CFG_FLASH_BASE+0x2FFFF);
