...
restart:
	...
	for (;;) {		
		 ...
		 */
			eth_rx();
		/* flash led for sercomm function */
		{
			static unsigned char blinktag = 0;
			static unsigned long oldtimer = 0;
			
			if (packetHandler == AssignHandler)
			{
				if((get_timer(0)-oldtimer)>(CFG_HZ*3/10))
				{
					if(blinktag){
						Led_on(POWER_LED);
						Led_on(SECURITY_LED);
						...
						Led_off(POWER_LED);
						Led_off(SECURITY_LED);
						blinktag = 1;
					}
					oldtimer = get_timer(0);
				}
			}
			else if (packetHandler == DownloadHandler)
			{
				if((get_timer(0)-oldtimer)>(CFG_HZ*3/10))
				{
					if(blinktag){
...
NetReceive(volatile uchar * inpkt, int len)
{
	...
	/* david: handle sercomm pkt */
	if ((packetHandler == AssignHandler)
		|| (packetHandler == DownloadHandler))
	{

		if ((x == 0x0) || (x == UPGRADE_HW_ETHER)	//sercomm download pkt
			|| (x == ASSIGN_HW_ETHER))	//sercomm assign pkt
		{
			//printf("sc packet received type=0x%x\n", x);
			//dumpData(inpkt, len);
