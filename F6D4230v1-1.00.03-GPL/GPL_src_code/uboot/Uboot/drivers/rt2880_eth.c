...
extern ulong rt2880_uboot_code_start_addr;
extern ulong *rt2880_top_dram_addr;
...
#define RT2880_MDIO_ACCESS_R  le32_to_cpu(*(volatile u_long *)(RALINK_FRAME_ENGINE_BASE+RT2880_MDIO_ACCESS))
#define RT2880_MDIO_CFG_R  le32_to_cpu(*(volatile u_long *)(RALINK_FRAME_ENGINE_BASE+RT2880_MDIO_CFG))

#define RT2880_PIODIR_R  le32_to_cpu(*(volatile u_long *)(RALINK_PIO_BASE + 0X24))
#define RT2880_PIODATA_R  le32_to_cpu(*(volatile u_long *)(RALINK_PIO_BASE + 0X20))
...
void RESET_RT2880_ETH(struct eth_device *dev)
{
...
#if 0
	u32 i;
	i=INL(dev, RT2880_FE_RST_GLO);
	//puts("\n RESET_RT2880_ETH is called 1 !!\n");
	udelay(100);
	OUTL(dev, i | RT2880_PSE_RESET ,RT2880_FE_RST_GLO);
	//puts("\n RESET_RT2880_ETH is called 2 !!\n");
	udelay(100);

	i=INL(dev, RT2880_PDMA_RST_IDX);
	//puts("\n RESET_RT2880_ETH is called 3 !!\n");
	udelay(100);
	OUTL(dev, i | RT2880_PST_DRX_IDX0 | RT2880_PST_DTX_IDX1 |  RT2880_PST_DTX_IDX0  ,RT2880_PDMA_RST_IDX);

	//puts("\n RESET_RT2880_ETH is called 4 !!\n");
	udelay(100);
#endif
	value = le32_to_cpu(*(volatile u_long *)(RALINK_SYSCTL_BASE + 0x0034));
	//printf("%x -> (%x)\n", addr+dev->iobase, value);

	//printf("\n 1.addr[RALINK_SYSCTL_BASE + 0x0034]=%08X !!\n",value);
	udelay(100);
	...
	*(volatile u_long *)(RALINK_SYSCTL_BASE + 0x0034) = cpu_to_le32(value);
	...
	udelay(200);
	value = le32_to_cpu(*(volatile u_long *)(RALINK_SYSCTL_BASE + 0x0034));
	//printf("\n 2.addr[RALINK_SYSCTL_BASE + 0x0034]=%08X !!\n",value);
}
...
u32 Gsw_Read_PHY(u32 phy_register, u32 *read_data)
{
	u32 volatile  status;
	u32 volatile  ii,t_start;
...
u32 Gsw_Write_PHY(u32 phy_register, u32 write_data)
{
	u32 volatile  ii,t_start;
	...
	RT2880_MDIO_ACCESS_R = 0;
	ii = 0;
	ii = (0x1F << 24) | (phy_register << 16) | (1 << 30) | write_data;
...
#if 0
static void gmac_phy_switch_gear(int i)
{
	u32 regValue;

	if(i == ENABLE)
	{
		Gsw_Read_PHY(0,&regValue);
		regValue &= ~(1<<11);
		Gsw_Write_PHY(0,regValue);
		printf("\n GMAC Phy Power Up \n");
	}
	else
	{
...
int rt2880_eth_initialize(bd_t *bis)
{
	...
	*read_data |= (1 << 15 );
	...
	}
	{
		...
		my_tmp = *(unsigned long *)(RALINK_FRAME_ENGINE_BASE + 0x0008);
		*(unsigned long *)(RALINK_FRAME_ENGINE_BASE + 0x0008) = my_tmp | 0x0000ff00;
	}
	return 1;
}
...
static int rt2880_eth_init(struct eth_device* dev, bd_t* bis)
{
	...
	while(1)
	{
		if (!(INL(dev, RT2880_MDIO_CFG) & (RT2880_GP1_LNK_DWN))) {
			printf("GMAC1 link up !!.\n");
			udelay(500);
			break;
		}
	}
	...
	while(1)
	{
		if (!(INL(dev, RT2880_MDIO_CFG) & (RT2880_GP1_AN_FAIL))) {
			printf("Auto-negotiation Finish !!.\n");
			udelay(500);
			break;
		}
	}
...
#if 1 //defined (RT2880_ASIC_BOARD) | defined (RT2883_ASIC_BOARD) || defined (RT3052_ASIC_BOARD)
static void rt2880_mdio_set(u32 addr,u32 bit_offset,u32 value)
{
	u32 temp;

	if(Gsw_Read_PHY(addr,&temp)) {
	}
	else {
...
inline static void force_mode(struct eth_device* dev)
{
#ifdef MAC_TO_GIGAMAC_MODE
	u32 regValue;
	regValue = RT2880_MDIO_CFG_R;
	regValue = regValue & 0xFFFF01FF;

	regValue |= (0xD << 12) | (0xD << 8);

	OUTL(dev, cpu_to_le32(regValue), RT2880_MDIO_CFG);
	regValue = RT2880_MDIO_CFG_R;
#endif // MAC_TO_GIGAMAC_MODE //
}


inline static void LANWANPartition(void)
{
...
#if defined (MAC_TO_GIGAMAC_MODE) && (defined (RT3052_ASIC_BOARD) || defined (RT3052_FPGA_BOARD))
...
#endif
#endif // MAC_TO_GIGAMAC_MODE && (RT3052_ASIC_BOARD || RT3052_FPGA_BOARD)
}

inline static void ResetSWusingGPIO10(void)
{
#ifdef GPIO10_RESET_MODE
	printf("\n GPIO pin 10 reset to switch\n");
	le32_to_cpu(*(volatile u_long *)RT2880_GPIOMODE_REG) |= (1 << 1);
	RT2880_PIODIR_R |= (1 << 10);

	//Set Gpio pin 10 to low
	RT2880_PIODATA_R &= ~(1 << 10);
	udelay(50000);
...
inline static int isMarvellGigaPHY(void)
{
#if defined (MAC_TO_GIGAPHY_MODE) 
	u32 phy_id0,phy_id1;

	if( ! Gsw_Read_PHY(2,&phy_id0)){
		printf("\n Read PhyID 0 is Fail!!\n");
		phy_id0 =0;
	}

	if( ! Gsw_Read_PHY(3,&phy_id1)){
...
{
#if defined (MAC_TO_GIGAPHY_MODE) || defined(RT3052_ASIC_BOARD) || defined (RT3052_FPGA_BOARD)
	u32 regValue;
...
static int isDMABusy(struct eth_device* dev)
{
	u32 kk;
...
#if 0
	*(volatile u_long *)(0x00 + RALINK_SYSCTL_BASE + 0x0700) = (1<<8)|(1<<0);
	*(volatile u_long *)(0x20 + RALINK_SYSCTL_BASE + 0x0700) = (1<<8)|(1<<0);
	udelay(500);
#endif
...
#else
		rt2880_mdio_set(20,7,1);
		rt2880_mdio_set(0,15,1);
#endif

		my_tmp = *(unsigned long *)(RALINK_FRAME_ENGINE_BASE + 0x8);
		*(unsigned long *)(RALINK_FRAME_ENGINE_BASE + 0x8) = my_tmp | 0x0000ff00;
	...
	OUTL(dev, cpu_to_le32((u32)(0x1001BC01)), RT2880_MDIO_CFG);
...
#endif // (RT2880_ASIC_BOARD) || (RT2883_ASIC_BOARD) || (RT3052_ASIC_BOARD)

	//puts("\n Enable GMAC Phy !!\n");
	//gmac_phy_switch_gear(ENABLE);

#if defined (MAC_TO_100PHY_MODE) || defined (MAC_TO_GIGAPHY_MODE)
	t_start = get_timer(0);
	t_start1 = t_start;
	...
	while(1) {
		...
		*/
		...
		if(get_timer(t_start)>= (2 * CFG_HZ)){
			t_start = get_timer(0);
			printf("\n Waitting..Current status is Link Down !!\n");
		}

#ifndef RT3052_FPGA_BOARD
		if(get_timer(t_start1)>= (20UL * CFG_HZ)){
			puts("\n Warning : Break waitting for the liuk up!!\n");
			break;
		}
#endif // non RT3052_FPGA_BOARD //
	}
#if defined MAC_TO_GIGAPHY_MODE
	t_start = get_timer(0);
	t_start1 = t_start;

	printf("Waitting  to Auto-negotiation Finish Start \n");
	...
	while(1){
		...
		if(get_timer(t_start)>= (2 * CFG_HZ)){
			regValue = RT2880_MDIO_CFG_R;
			t_start = get_timer(0);
			printf("\n Waitting..Current status is Auto-negotiation  0x%08x!!\n", regValue);
		}

		if(get_timer(t_start1)>= (7 * CFG_HZ)){
			printf(" Warning : Break waitting for the AN completion !!(Timeout) \n");
			break;
		}
	}
#endif // MAC_TO_GIGAPHY_MODE //
...
#if 0
	regValue = RT2880_MDIO_CFG_R;
	regValue= regValue & (~(RT2880_BIT(16)));
	RT2880_MDIO_CFG_R = regValue;
#endif

#endif /* (MAC_TO_100PHY_MODE) || (MAC_TO_GIGAPHY_MODE) */
...
static int rt2880_eth_recv(struct eth_device* dev)
{
	...
	if (rt3052_phy_test == PHY_TEST_ENABLE) {
		...
		while(length == 0)
		{
			...
			if(length != 0)
			{
				printf("\n Warring!! Header length > Payload length!!rxd_info2.PLEN=%d ,start_t=%ld\n",length,get_timer(0));
			}
			if(get_timer(start_t) > CFG_FLASH_STATE_DISPLAY_TOUT)
			{
				printf("\n kaiker!!!! rx_ring[rx_dma_owner_idx0].rxd_info2.PLEN=%d\n",rx_ring[rx_dma_owner_idx0].rxd_info2.PLEN0);
				return (0);
			}
		...
		//printf("\n Rcv the packet,that addr=[0x%08X] \n",NetRxPackets[rx_dma_owner_idx0]);
...
u32 mii_mgr_read(u32 phy_addr, u32 phy_register, u32 *read_data)
{
	...
	//MDIO_PHY_CONTROL_0= 0;
	//MDIO_PHY_CONTROL_1= 0;
	ii = 0;
	ii = (phy_addr) | (phy_register << 8) | 1<<14;

	MDIO_PHY_CONTROL_0 = ii;
	//printf("\n Set Command [0x%08X] to PHY !!\n",ii);
...
u32 mii_mgr_write(u32 phy_addr, u32 phy_register, u32 write_data)
{
	...
	//RT2880_MDIO_ACCESS_R = 0;
	ii = 0;
	ii = (phy_addr) | (phy_register << 8) | (1<<13) | ((write_data&0xffff)<<16);
...
#endif // defined (RT3052_FPGA_BOARD) || defined (RT3052_ASIC_BOARD)
...
int rt2880_mdio_access(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	{
		value = RT2880_MDIO_CFG_R;
		value |= RT2880_BIT(15);
		RT2880_MDIO_CFG_R = value;
...
int rt3052_switch_command(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (argc == 1 || !strncmp(argv[1], "dump", 5)) { //switch dump
		...
		printf("hash  cpu  port(0:6)  vid  age  pxy  in   mac-address\n");
		for (i = 0; i < 0x400; i++) {
			while(1) {
				...
				if (value & 0x1) { //search_rdy
					...
					printf("  %c  ", (value & 0x400)? 'y':'n'); //r_cpu_port
