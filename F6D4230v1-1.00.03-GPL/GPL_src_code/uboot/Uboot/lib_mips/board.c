...
#define RT3052_MP2 /* wanles add for IOT 2008-09-24 */
...
#if 1	
	gpio_init();  /* wanles fix gpio init, 2008-07-30 */
	timer_init();
...
//   printf("   %d: Load Boot Loader code to SDRAM via TFTP. \n", SEL_LOAD_BOOT_SDRAM);
   printf("   %d: Load Boot Loader code then write to Flash via TFTP. \n", SEL_LOAD_BOOT_WRITE_FLASH);
	printf("   a: Sercomm Assign. \n");
	printf("   d: Sercomm Download. \n");
}
...
unsigned char sc_protect_buf[SC_PROTECT_END - SC_PROTECT_START];
...
#ifndef RT3052_MP2  /* wanles changed 2008-09-24 */
...
#endif  /* end of wanles changed 2008-09-24 */
/* PHY IOT */
	mii_mgr_write(0, 31, 0x0);	//---> select global register
#ifndef RT3052_MP2
	mii_mgr_write(0, 18, 0x40da);	//---> improve IOT parameter
#else
...
/* wanles move this block here for fix the timer of support VLAN, 2008-08-22 */
...
//      sprintf(addr_str, "0x%X", CFG_FLASH_BASE + 0x50000);
      sprintf(addr_str, "0x%X", CFG_FLASH_BASE + 0x40000);  /* wanles changed for kernel start 0x40000 2008-07-07 */
      argv[1] = &addr_str[0];
...
/* by bruce */
	    ...
	    //keep data
	    memcpy(sc_protect_buf, (void *)SC_PROTECT_START, (SC_PROTECT_END - SC_PROTECT_START));
	    ...
	    //write data back
	    if (flash_write(sc_protect_buf, SC_PROTECT_START, (SC_PROTECT_END - SC_PROTECT_START)))
	    {
