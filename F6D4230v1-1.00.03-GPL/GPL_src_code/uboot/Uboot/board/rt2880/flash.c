...
unsigned long flash_size = 0x0; /* wanles,2008-08-22 */
...
static void flash_reset(flash_info_t *info)
{
...
#ifdef FLASH_SWAP
		*base = (FPW)0xFF00FF00;	/* Intel Read Mode */
#else
	...
	{
#ifdef FLASH_SWAP
		*base = (FPW)0x0F000F00;	/* AMD Read Mode */
#else
	...
	{
#ifdef FLASH_SWAP
		*base = (FPW)0x0F000F00;	/* MX Read Mode */
#else
...
#ifdef FLASH_SWAP
	addr[FLASH_CYCLE1] = (FPW)0x55005500;	/* for AMD, Intel ignores this */
	addr[FLASH_CYCLE2] = (FPW)0xAA00AA00;	/* for AMD, Intel ignores this */
	addr[FLASH_CYCLE1] = (FPW)0x09000900;	/* selects Intel or AMD */
#else
  ...
  //   printf("\n The Flash ID =%08X \n",addr[1]);
...
#ifdef FLASH_SWAP
	switch (addr[0] & 0xff00ff00) {
#else
	switch (addr[0] & 0xff) {
#endif

#ifdef FLASH_SWAP
	case (FPW)AMD_MANUFACT:
#else
	...
	if (info->flash_id != FLASH_UNKNOWN)
		...
		case (FPW)INTEL_ID_28F160C3B:
			...
			flash_size = 0x00200000;     /* wanles, 2008-08-22 */
			break;				/* => 2 or 4 MB		*/
		...
		case (FPW)INTEL_ID_28F320C3B:
			...
			flash_size = 0x00400000;     /* wanles, 2008-08-22 */
			break;				/* => 4 or 8 MB		*/
...
#ifndef FLASH_SWAP
		case (FPW)MX_ID_29LV160DB:    /* wanles add for support MX29lv160DBTI-70g,2008-08-22 */
			info->flash_id += MX_ID_29LV160DB;
		 	info->sector_count = 39;
			info->size = 0x00200000 * (sizeof(FPW)/2);
			flash_size = 0x00200000;
			printf("\n MX_ID_29LV160DB, Size = %08x bytes\n",info->size);
			break;
#endif
...
int erase_all_chip(flash_info_t *info, int s_first, int s_last)
{
	...
	case EN_ID_29LV640H:	
#ifndef FLASH_SWAP
	case MX_ID_29LV160DB: /* wanles,2008-08-22 */
#endif
		break;
	case FLASH_UNKNOWN:
	default:
...
//	for (sect = s_first; sect<=s_last && rcode == 0; sect++) 
	{
...
#ifdef FLASH_SWAP
			base[FLASH_CYCLE1] = (FPW)0x55005500;	/* unlock */
			base[FLASH_CYCLE2] = (FPW)0xAA00AA00;	/* unlock */
			base[FLASH_CYCLE1] = (FPW)0x01000100;	/* erase mode */
			base[FLASH_CYCLE1] = (FPW)0x55005500;	/* unlock */
			base[FLASH_CYCLE2] = (FPW)0xAA00AA00;	/* unlock */
			base[FLASH_CYCLE1] = (FPW)0x08080808;   /* erase all */
#else
		...
		poll = 0;
#ifdef FLASH_SWAP
		while ((*addr & (FPW)0x01000100) != (FPW)0x01000100) {
#else
...
#ifndef FLASH_SWAP
	case MX_ID_29LV160DB:  /* wanels 2008-08-22 */
#endif
		break;
	case FLASH_UNKNOWN:
	default:
	...
	//	printf("\n erase sector  = %d \n",sect);
		...
		if( sect == 0 || flash_use_SSI_standard == 1)
		{
			...
			for(kk =0 ; kk < s_kk ;kk++)
			{
				...
				base[FLASH_CYCLE1] = (FPW)0x55005500;	/* unlock */
				base[FLASH_CYCLE2] = (FPW)0xAA00AA00;	/* unlock */
				*addr = (FPW)0x0C000C00;	/* erase sector */
#else
				base[FLASH_CYCLE1] = (FPW)0x00AA00AA;	/* unlock */
				base[FLASH_CYCLE2] = (FPW)0x00550055;	/* unlock */
				base[FLASH_CYCLE1] = (FPW)0x00800080;	/* erase mode */
		...
		if (intel) {
#ifdef FLASH_SWAP
			*addr = (FPW)0x0A000A00; /* clear status register */
			*addr = (FPW)0x04000400; /* erase setup */
			*addr = (FPW)0x0B000B00; /* erase confirm */
#else
...
//	printf("\n *dest = %08X\n",*dest);
	...
	*/
	...
	base[FLASH_CYCLE1] = (FPW)0x55005500;	/* unlock */
	base[FLASH_CYCLE2] = (FPW)0xAA00AA00;	/* unlock */
	base[FLASH_CYCLE1] = (FPW)0x05000500;	/* selects program mode */
#else
	base[FLASH_CYCLE1] = (FPW)0x00AA00AA;	/* unlock */
	base[FLASH_CYCLE2] = (FPW)0x00550055;	/* unlock */
...
//	printf("[%d] Toggle %d, %d\n", poll, (status  ^ oldstatus) & dq6, (status ^ oldstatus) & dq2);
	...
	while( res ==0 && (status & dq6) != (oldstatus & dq6) && 
		...
		if (get_timer(start) > CFG_FLASH_WRITE_TOUT) {
#ifdef FLASH_SWAP
			*dest = (FPW)0x0F000F00;	/* reset bank */
#else
			*dest = (FPW)0x00F000F0;	/* reset bank */
#endif
			printf("*reset bank\n");	 
