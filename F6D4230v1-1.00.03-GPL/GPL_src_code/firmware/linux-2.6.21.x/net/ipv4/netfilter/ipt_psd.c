	      ...
	      int offset, unsigned int protoff, int *hotdrop)
{
	...
	if (curr) {
		...
		if (now - curr->timestamp <= (psdinfo->delay_threshold*HZ)/100 &&
			...
			/* asraul, Jul 6, 2006, log TCP SCAN */
			if (proto == IPPROTO_TCP && (tcp_hdr->ack || tcp_hdr->rst) && !tcp_hdr->syn)
				goto out_no_match;
	...
	/* asraul, Jul 6, 2006, log TCP SCAN */
	if (proto == IPPROTO_TCP && tcp_hdr->ack && !tcp_hdr->syn)
		goto out_no_match;
...
{
	return xt_register_match(&ipt_psd_reg);
}
